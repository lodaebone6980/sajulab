...
5. 고객의 이름을 사용하여 개인화된 분석서 작성 (예: "홍길동님", "석현주님")
6. "~하세요", "~해요", "~이에요/예요" 체 사용 (부드럽고 친근한 어조). 고객을 반드시 실제 이름으로 호칭 (예: "홍길동님"). "의뢰자님", "귀하" 등 일반적 호칭은 절대 사용하지 마세요.
...
응답 형식 (반드시 JSON):
{
  "greeting": "${customerName}님께 보내는 따뜻한 인사말. 반드시 ${customerName}님으로 호칭. (300~400자)",
  "chapters": [
    {
      "number": "01",
      "title": "종합 운세 분석",
      "content": "종합적인 사주 분석. 일간의 기질과 의미, 오행 균형, 용신의 역할, 성격적 특징, 직업 적성, 재물운, 건강 주의점, 연애/가정운, 대인관계, 개운법을 상세하고 구체적으로 서술. ${customerName}님의 사주 데이터에 근거하여 풍부하고 깊이 있게 분석하세요. 각 분야별로 구체적인 조언과 실천 방안을 반드시 포함하세요. (3500~4500자)"
    }
  ]
}`;
}
...
⚠️ 매우 중요: 각 챕터는 반드시 최소 8000자 이상이어야 합니다. 짧은 답변은 절대 허용되지 않습니다.
각 챕터를 작성할 때 다음을 모두 포함하세요: 1)원리 설명 2)구체적 사례 3)비유와 예시 4)실천 방안 5)주의사항 6)월별/시기별 세부분석 7)다른 요소와의 상호작용.
최소 글자수에 도달할 때까지 내용을 풍부하게 확장하세요. 절대 요약하지 마세요.
고객을 반드시 "${customerName}님"으로 호칭하세요. "의뢰자님"이라는 호칭은 절대 사용하지 마세요.
...
응답 형식 (반드시 JSON):
{
  "greeting": "${customerName}님께 보내는 따뜻한 인사말 (500~800자). 반드시 ${customerName}님으로 호칭.",
  "chapters": [
    {
      "number": "01",
      "title": "나의 사주팔자 상세분석",
      "content": "일간의 근본 특성과 성격적 의미를 깊이 있게 분석. 사주 구성의 독특한 조합이 어떤 의미를 갖는지, 오행 균형 상태와 그 영향, 강약 판정의 의미, 일간과 다른 천간/지지의 상호작용을 구체적 예시와 함께 서술. 십성 배치의 의미, 십이운성 분석, 일간의 기질이 일상생활/대인관계/의사결정에 어떻게 나타나는지 상세히 설명. 신살 분석과 형충회합의 영향까지 포함하여 ${customerName}님의 타고난 성격과 운명의 기본 틀을 폭넓게 다루세요. (8000~10000자)"
    },
    {
      "number": "02",
      "title": "내 인생의 황금기",
      "content": "대운(大運)의 개념과 ${customerName}님의 전체 대운 흐름을 시기별로 심층 분석. 각 대운 시기(10년 단위)의 특징, 기회, 도전을 설명. 인생에서 가장 중요한 황금기 시점과 그 의미, 현재 대운의 특성, 앞으로 남은 대운 전망을 설명. 대운 전환기에 주의할 점, 오행의 흐름에 따른 에너지 변화, 용신과 기신이 대운에서 어떻게 작용하는지, 각 시기별 구체적 실천 전략을 포함. 십이운성의 흐름과 인생 사이클 분석, 과거 대운의 특성과 앞으로의 전환점을 상세하게 서술. (8000~10000자)"
    },
    {
      "number": "03",
      "title": "연애운과 배우자운",
    ...
    },
    {
      "number": "08",
      "title": "운명을 바꾸는 방법",
      "content": "용신(用神)과 희신의 개념과 ${customerName}님에게 이로운 기운의 정체를 설명. 용신 오행을 일상에서 활용하는 구체적 방법(색깔, 방향, 숫자, 음식, 직업, 취미 등), 풍수 인테리어 조언(집/사무실), 행운의 아이템과 액세서리. 이름이나 호에서의 오행 활용, 기신을 피하는 방법, 개운법의 원리, 부적/명상/기도의 효과, 사주의 약점을 보완하는 생활 습관, 일상에서 실천 가능한 구체적 개운 루틴을 포함하여 서술. (8000~10000자)"
    },
    {
      "number": "09",
      "title": "월별 상세 운세",
...
⚠️ 매우 중요: 각 챕터는 반드시 최소 4000자 이상이어야 합니다. 짧은 답변은 절대 허용되지 않습니다.
각 챕터를 작성할 때 다음을 모두 포함하세요: 1)원리 설명 2)구체적 사례 3)비유와 예시 4)실천 방안 5)주의사항 6)월별 세부 흐름 7)다른 운과의 연관성.
최소 글자수에 도달할 때까지 내용을 풍부하게 확장하세요. 절대 요약하지 마세요.
...
응답 형식 (반드시 JSON):
{
  "greeting": "${customerName}님께 보내는 ${year}년 새해 인사말. 사주의 특성과 새해 전망을 포함한 따뜻한 인사. 반드시 ${customerName}님으로 호칭. (400~600자)",
  "chapters": [
    {
      "number": "01",
      "title": "나의 사주 한눈에 보기",
...
// ─── Chapter Definitions for Premium and NewYear Products ───

export function getPremiumChapterDefinitions(customerName: string): ChapterDefinition[] {
  return [
    {
      number: '01',
      title: '나의 사주팔자 상세분석',
      guide: `일간의 근본 특성과 성격적 의미를 깊이 있게 분석. 사주 구성의 독특한 조합이 어떤 의미를 갖는지, 오행 균형 상태와 그 영향, 강약 판정의 의미, 일간과 다른 천간/지지의 상호작용을 구체적 예시와 함께 서술. 십성 배치의 의미, 십이운성 분석, 일간의 기질이 일상생활/대인관계/의사결정에 어떻게 나타나는지 상세히 설명. 신살 분석과 형충회합의 영향까지 포함하여 ${customerName}님의 타고난 성격과 운명의 기본 틀을 폭넓게 다루세요. (8000~12000자)`,
    },
    {
      number: '02',
      title: '내 인생의 황금기',
      guide: `대운(大運)의 개념과 ${customerName}님의 전체 대운 흐름을 시기별로 심층 분석. 각 대운 시기(10년 단위)의 특징, 기회, 도전을 설명. 인생에서 가장 중요한 황금기 시점과 그 의미, 현재 대운의 특성, 앞으로 남은 대운 전망을 설명. 대운 전환기에 주의할 점, 오행의 흐름에 따른 에너지 변화, 용신과 기신이 대운에서 어떻게 작용하는지, 각 시기별 구체적 실천 전략을 포함. (8000~12000자)`,
    },
    {
      number: '03',
      title: '연애운과 배우자운',
      guide: `사주에서 읽히는 연애 패턴과 이상형, 배우자 궁(일지)의 의미, 배우자의 성격과 외모 유형, 연애 시기와 결혼 적기, 결혼 후의 부부 관계 전망을 깊이 있게 분석. 연애 시 강점과 주의할 약점, 이별이나 갈등의 원인과 해결법, 궁합이 좋은 사주 유형, 연애운을 높이는 구체적 방법. (8000~12000자)`,
    },
    {
      number: '04',
      title: '나의 재물운 분석',
      guide: `재성(편재/정재)의 위치와 강약 분석, 재물 획득 패턴(정기 수입형/투자형/사업형/횡재형), 돈에 대한 태도와 소비 습관, 재테크 적성을 상세히 분석. 부동산/주식/사업 중 유리한 재물 경로, 시기별 재물운의 흐름(대운/세운 기준), 재물을 잃기 쉬운 시기와 원인, 재물운을 극대화하는 전략. (8000~12000자)`,
    },
    {
      number: '05',
      title: '직업과 성공의 운명',
      guide: `사주에서 나타나는 직업 적성과 재능, 관성(편관/정관)과 식상(식신/상관)의 배치가 직업에 미치는 영향을 심층 분석. 적합한 직업 분야와 그 이유, 사업가 기질 분석, 리더십 스타일, 직장 내 인간관계 패턴, 승진/이직 시기 분석, 성공을 위한 구체적 전략. (8000~12000자)`,
    },
    {
      number: '06',
      title: '사주로 보는 건강과 체질',
      guide: `오행 기반 체질 분석(어떤 장기가 강하고 약한지), 선천적으로 주의해야 할 질환, 나이별 건강 변화 전망을 상세히 분석. 계절별 건강 관리 포인트, 오행별 추천 음식과 피해야 할 음식, 운동 추천, 정신 건강. (8000~12000자)`,
    },
    {
      number: '07',
      title: '당신을 도와줄 운명의 귀인',
      guide: `사주에 나타난 귀인(貴人)의 유형과 특징, 귀인을 만나는 시기와 장소, 신살(神殺) 분석을 통한 특수한 인연. 도움이 되는 사람의 띠/나이/직업 유형, 가족 관계 분석, 대인관계 패턴, 사회적 네트워킹 전략. (8000~12000자)`,
    },
    {
      number: '08',
      title: '운명을 바꾸는 방법',
      guide: `용신(用神)과 희신의 개념과 ${customerName}님에게 이로운 기운의 정체를 설명. 용신 오행을 일상에서 활용하는 구체적 방법(색깔, 방향, 숫자, 음식, 직업, 취미 등), 풍수 인테리어 조언, 행운의 아이템, 개운 루틴. (8000~12000자)`,
    },
    {
      number: '09',
      title: '월별 상세 운세',
      guide: `올해 1~12월 각 월의 천간지지와 오행, 사주와의 상호작용을 상세 분석. 각 월별로 전반적 운세, 재물운, 직업운, 연애운, 건강 주의점, 해야 할 일과 피해야 할 일을 포함하여 800~1000자씩 작성. (10000~14000자)`,
    },
    {
      number: '10',
      title: '앞으로의 10년간 운명 분석',
      guide: `올해부터 향후 10년간의 세운(歲運) 흐름을 연도별로 심층 분석. 각 연도별 천간지지, 사주와의 관계, 주요 이벤트 전망, 기회와 위험, 재물/직업/건강/연애 각 분야 전망을 800~1000자씩 연도별 작성. (10000~14000자)`,
    },
  ];
}

export function getNewYearChapterDefinitions(customerName: string, year: number): ChapterDefinition[] {
  return [
    {
      number: '01',
      title: '나의 사주 한눈에 보기',
      guide: `사주 핵심 특성을 상세하게 설명. 일간의 의미와 성격적 특징, 오행 균형 상태의 의미, 용신과 기신이 삶에 미치는 영향, 사주에서 나타나는 강점과 약점, 십성 배치 분석, 신살의 영향. (5000~7000자)`,
    },
    {
      number: '02',
      title: `${year}년 운세 총평`,
      guide: `${year}년 세운(歲運)의 특성, 사주와의 관계, 올해의 핵심 테마와 키워드, 상반기와 하반기의 운기 흐름 차이, 올해 특히 주목해야 할 포인트, 전반적인 운의 방향성, 대운과 세운의 교차 효과. (6000~8000자)`,
    },
    {
      number: '03',
      title: `${year}년 재물운`,
      guide: `${year}년 재물운의 전반적 흐름, 수입과 지출의 패턴, 재테크나 투자에 유리한 시기와 불리한 시기, 예상치 못한 재물 기회, 재물운을 높이기 위한 구체적 실천 방안, 월별 재물운 흐름, 부동산/주식/사업 전망. (5000~7000자)`,
    },
    {
      number: '04',
      title: `${year}년 직업/사업운`,
      guide: `${year}년 직업운과 사업운의 흐름, 승진이나 이직에 유리한 시기, 사업 확장의 적기, 동료나 상사와의 관계 변화, 커리어 발전을 위한 구체적 전략과 조언. (5000~7000자)`,
    },
    {
      number: '05',
      title: `${year}년 연애/가정운`,
      guide: `${year}년 연애운의 흐름, 새로운 만남의 가능성과 시기, 기존 관계의 변화, 결혼운, 가족관계의 변동, 배우자나 파트너와의 조화, 애정운을 높이기 위한 실천 방안. (5000~7000자)`,
    },
    {
      number: '06',
      title: `${year}년 건강운`,
      guide: `${year}년 건강 운세와 주의해야 할 질환, 계절별 건강 관리 포인트, 오행 기반 체질 보강법, 정신 건강과 스트레스 관리법, 운동과 식이 조언. (5000~7000자)`,
    },
    {
      number: '07',
      title: `${year}년 인간관계와 귀인`,
      guide: `${year}년 인간관계의 변화, 귀인을 만나는 시기와 장소, 가족/친구/동료와의 관계 전망, 주의해야 할 소인, 좋은 인연을 끌어당기는 방법. (5000~7000자)`,
    },
    {
      number: '08',
      title: '주의할 시기와 대비법',
      guide: `${year}년 중 특히 주의해야 할 시기(월별), 위험 요소와 대비 전략, 사주의 약점이 드러나는 시기, 차분하게 넘길 수 있는 현실적 조언. (5000~7000자)`,
    },
    {
      number: '09',
      title: `${year}년 행운 가이드 및 개운법`,
      guide: `${year}년 행운색, 행운 숫자, 행운 방향, 용신 활용 구체적 개운법, 풍수 인테리어 팁, 행운 아이템, 일상 개운 루틴, 이름/호에서의 오행 활용법. (5000~7000자)`,
    },
    {
      number: '10',
      title: `${year}년 월별 상세 운세`,
      guide: `${year}년 1~12월 각 월의 천간지지와 오행, 사주와의 상호작용을 상세 분석. 각 월별로 전반적 운세, 재물운, 직업운, 연애운, 건강 주의점을 포함하여 800~1000자씩 작성. (10000~14000자)`,
    },
  ];
}
